...
• visits have queue information, and connect source_id to location_id. There were multiple visits to sites, so we need to be careful to
...
But the visits table maps location_id and source_id. So if we use visits as the table we query from, we can join the location where
the location_id matches, and water_source where the source_id matches.
Before we can analyse, we need to assemble the data into a table first. It is quite complex, but once we're done, the analysis is much simpler!*/
...
*/
Date_of_completion DATE, -- Engineers will add this on the day the source has been upgraded.
Comments TEXT -- Engineers can leave comments. We use a TEXT type that has no limit on character length
);
