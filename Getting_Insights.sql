...
-- replace the space with . to get a proper email format 
SELECT
REPLACE(employee_name, ' ','.')  
FROM
employee;

-- change the name to lower case, emails are normally written in lower case
SELECT
LOWER(REPLACE(employee_name, ' ','.'))  /* Make it all lower case*/
FROM
employee;

-- combine the above scripts and add the extension to get a full email 
...
-- Disable safe update mode to be able to update a table 
...
-- the length of phone numbers should be 12. We see that we have 13 here 
SELECT
LENGTH(phone_number)
FROM
employee;

-- let's trim to remove spaces and get the right phone numbers. Our SMS should work perfectly 
...
-- see employee names and the towns they live in 
SELECT employee_name, town_name
FROM employee;

-- lets see how many of our employees live in each town 
SELECT town_name, COUNT(*) AS employee_count
FROM employee
GROUP BY town_name;

-- The number of our employees living in rural areas
SELECT 
...
-- We can go find out their names using the employee IDs
...
-- Let's see our top 3 employees
-- They visited the most locations. Let's include their contact information. We can decide to award them
SELECT 
...
-- Let's group towns and their number of records 
-- To avoid wrong results, let's group provinces first, then group individual towns per province and get the records per town 
...
-- just trying arithmetics to get a % here 
...
-- Let's rank the water sources according to the number of people using the water 
SELECT
type_of_water_source,
SUM(number_of_people_served) AS number_of_people,
...
-- ranking the different sources. If we have 25 rivers and 30 wells, I will rank the rivers, then rank the wells and so on 
SELECT
  source_id,
  type_of_water_source,
...
-- Let's break down the information into every hour 
SELECT
...
-- Use the CASE function to get every hour of every day, weekly
SELECT
TIME_FORMAT(TIME(time_of_record), '%H:00') AS hour_of_day,
ROUND(AVG(CASE
...
-- Seeing what different date formats will look like
...
-- How many visits did each employee make? 
