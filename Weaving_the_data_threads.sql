...
-- import the auditors table, which is saved as a CSV file 
-- view the data saved by the auditor
SELECT * FROM auditor_report;

-- where the employee score and the auditor's quality score did not match 
SELECT
...
-- just another long query to get the name of the employee and the number of mistakes made by them 
SELECT employee_name, COUNT(*) AS mistake_count
FROM (
    SELECT 
        ar.location_id AS audit_location,
